...
#an example where carbon concentration is 10% (.1 gram/gram), with measurement error .001 grams. 1 gram is weighed at a resolution of .00001 (5 decimal places) or .000001 (6 decimal places). 
#sigma_M = .000005 puts 95% of the mass measurements within .00001 grams of the true mass.
#sigma_C = .0005 puts 95% of the carbon mass measurements made by the machine within .001 grams of the true carbon mass. 
sims_5_places <- run_sim(n = 1e6, mu_M = 1, var_M = .000005^2, mu_C = .1, var_C = .0005^2)
sims_6_places <- run_sim(n = 1e6, mu_M = 1, var_M = .0000005^2, mu_C = .1, var_C = .0005^2)
...
The simulated measurement error is about .0005 grams, so 95% of measurements are in about 0.001 gram/gram or .1 percent of the true concentration. 0.0005 grams is essentially just the error of the machine, so in this case the effect of more imprecise weighing is completely negligible. This also tracks with the analytical approximation:

$$0.1^2 \bigg( \frac{0.0005^2}{0.1^2} + \frac{0.00005^2}{1^2} \bigg) \bigg / 0.1^2 \bigg( \frac{0.0005^2}{0.1^2} + \frac{0.000005^2}{1^2} \bigg)$$
```{r}
(.0005^2 / .01 + .00005^2) / (.0005^2 / .01 + 0.000005^2)
```

The LHS inside the parentheses, which corresponds to the machine precision, dominates the variability of assay. Of course, other inputs for the true weight of the sample, the true concentration of the sample, or the precision of the machine will change the cost of using a 5 decimal over a 6 decimal point balance. These costs could be explored for a range of other choices of the parameters.
