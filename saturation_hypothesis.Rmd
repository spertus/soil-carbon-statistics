...
```{r}
#histogram across sites: variability in baseline SOC
ggplot(nrcs_data %>% filter(year == 2016), aes(TC, fill = treatment)) +
...
```{r boxplots of nrcs data}
ggplot(nrcs_data, aes(y = TC, x = site, fill = year)) +
  geom_boxplot() +
  facet_grid(treatment ~ .) 
```

```{r association of baseline and followup SOC}
baseline_followup <- nrcs_data %>%
  select(-TN) %>% 
  group_by(treatment, site, year) %>%
  summarize(TC = mean(TC)) %>%
  pivot_wider(names_from = "year", values_from = "TC", names_prefix = "TC_") 

ggplot(baseline_followup, aes(x = TC_2016, y = TC_2019)) +
...
```{r difference estimates each year}
#this is acting as though we have paired randomization at the core level, not the site level
yearly_estimates <- yearly_mean_effects %>% 
  group_by(year) %>%
  summarize(mean_diff = mean(diff), se_diff = sd(diff)/sqrt(n()))
```

```{r DiD estimate of ATE using plots}
#this (correctly) treats plots as the fundamental unit of randomization
estimates_plots <- nrcs_data %>% 
  select(-TN) %>% 
  group_by(treatment, site, year) %>%
  summarize(TC = mean(TC)) %>%
  pivot_wider(names_from = "year", values_from = "TC", names_prefix = "TC_") %>%
...
```{r plot followup against baseline at plot level}
#core level
plot_level_data <- nrcs_data %>% 
  select(-TN) %>% 
  group_by(treatment, site, year) %>%
  summarize(TC = mean(TC)) %>%
  pivot_wider(names_from = "year", values_from = "TC", names_prefix = "TC_") %>%
  mutate(diff = TC_2019 - TC_2016)
ggplot(plot_level_data, aes(x  = TC_2016, y = TC_2019, color = treatment)) +
  geom_point() + 
  scale_y_log10() +
  scale_x_log10()
```

```{r specious regression to the mean, message = FALSE}
#supposed regression to the mean effect is very pronounced, especially without BethelIsland
ggplot(plot_level_data, aes(x  = TC_2016, y = diff)) +
  geom_point(aes(color = treatment)) + 
  geom_smooth(method = "lm") +
  xlab("%TC 2016") +
  ylab("%TC 2019 - %TC 2016")

ggplot(plot_level_data %>% filter(site != "BethelIsland"), aes(x  = TC_2016, y = diff)) +
  geom_point(aes(color = treatment)) + 
  geom_smooth(method = "lm") +
  xlab("%TC 2016") +
  ylab("%TC 2019 - %TC 2016")
...
```{r plot level CATE}
B_grid <- seq(0,12,by=0.01) #grid of baseline SOC for plotting
alpha <- 0.05 #confidence level

#naive estimate of difference exhibiting "regression to the mean"
naive_estimate <- predict(lm(diff ~ TC_2016, data = plot_level_data), newdata = data.frame(TC_2016 = B_grid), interval = "confidence", level = 1-alpha)

#linear model estimates of CATE with confidence interval
mu_hat_0 <- predict(lm(TC_2019 ~ TC_2016, data = plot_level_data, subset = treatment == "C"), newdata = data.frame(TC_2016 = B_grid), interval = "confidence", level = 1-alpha/2)
mu_hat_1 <- predict(lm(TC_2019 ~ TC_2016, data = plot_level_data, subset = treatment == "T"), newdata = data.frame(TC_2016 = B_grid), interval = "confidence", level = 1-alpha/2)
tau_hat_ols <- cbind(
  "estimate" = mu_hat_1[,1] - mu_hat_0[,1], 
  "lower_bound" = mu_hat_1[,2] - mu_hat_0[,3],
  "upper_bound" = mu_hat_1[,3] - mu_hat_0[,2])

#plotting in base R
plot(0,0, type = "n", xlim = c(0,12), ylim = c(-4,4), xlab = "Baseline %TC", ylab = "CATE")
points(naive_estimate[,1] ~ B_grid, type = 'l', lwd = 2, lty = 'solid', col = 'red')
points(naive_estimate[,2] ~ B_grid, type = 'l', lwd = 1, lty = 'dashed', col = 'red')
points(naive_estimate[,3] ~ B_grid, type = 'l', lwd = 1, lty = 'dashed', col = 'red')
points(tau_hat_ols[,1] ~ B_grid, type = 'l', lwd = 2, lty = 'solid', col = 'blue')
points(tau_hat_ols[,2] ~ B_grid, type = 'l', lwd = 1, lty = 'dashed', col = 'blue')
points(tau_hat_ols[,3] ~ B_grid, type = 'l', lwd = 1, lty = 'dashed', col = 'blue')
```
...
N = 10 * nrow(nrcs_data) #population size = 2480 fields
```


```{r set up parameter grids for simulations}
...
measurement_noise <- round(c(assay_variability + baseline_wp_var/5, 
                       assay_variability + baseline_wp_var/30, 
                       assay_variability + baseline_wp_var/100), 2) #variability due to sampling / within-plot spatial heterogeneity. n = 5,30,100 samples per plot
...
run_simulation <- function(ate, moderator_effect, idiosyncratic_teh, measurement_noise, n){
  ...
  #key parameters to be estimated
  ...
  for(i in 1:num_sims){
    ...
    B <- b[sample_index] + rnorm(n, mean = 0, sd = sqrt(measurement_noise))
    B_std <- stdize(B)
    ...
    #using the Ding et al package "hettx" to estimate moderator effects
    ...
    # S_xx1 <- (2/n) * t(X_1) %*% X_1
    # S_xx0 <- (2/n) * t(X_0) %*% X_0
    # S_x1 <- (2/n) * Y[trt_index] %*% X_1
    # S_x0 <- (2/n) * Y[-trt_index] %*% X_0
    # gamma_1 <- solve(S_xx1) %*% t(S_x1)
    # gamma_0 <- solve(S_xx0) %*% t(S_x0)
    # e_1 <- Y[trt_index] - X_1 %*% gamma_1 
    # e_0 <- Y[-trt_index] - X_0 %*% gamma_0
    # mod_est_ding <- gamma_1[2] - gamma_0[2]
    # S_ex1 <- (1/(n/2 - 1)) * t(cbind(e_1, e_1)*X_1) %*% (cbind(e_1, e_1)*X_1) 
    # S_ex0 <- (1/(n/2 - 1)) * t(cbind(e_0, e_0)*X_0) %*% (cbind(e_0, e_0)*X_0) 
    # cov_mod_est <- solve(S_xx1) %*% (S_ex1/(n/2)) %*% solve(S_xx1) + solve(S_xx0) %*% (S_ex0/(n/2)) %*% solve(S_xx0)
    # mod_se_ding <- sqrt(cov_mod_est[2,2])
    # mod_ci_ding <- c(mod_est_ding - qnorm(0.975) * mod_se_ding, mod_est_ding + qnorm(0.975) * mod_se_ding)
...
for(i in 1:nrow(simulation_parameters)){
  all_results[[i]] <- run_simulation(simulation_parameters$ate[i], simulation_parameters$moderator_effect[i],  simulation_parameters$idiosyncratic_teh[i], simulation_parameters$measurement_noise[i], simulation_parameters$n[i])
}
...
mod_est_results <- results_frame %>% 
  group_by(n) %>%
  summarize(bias = mean(modest_bias), rmse = mean(modest_rmse), coverage = mean(modest_coverage), ciwidth = mean(modest_ciwidth))
```
...
all_results_n <- list()
for(i in 1:nrow(simulation_parameters)){
  all_results_n[[i]] <- run_simulation(simulation_parameters$ate[i], simulation_parameters$moderator_effect[i],  simulation_parameters$idiosyncratic_teh[i], simulation_parameters$measurement_noise[i], simulation_parameters$n[i])
}
...
```{r plot properties as a function of n}
...
```{r moderator effects}
mod_estimates <- results_frame %>% 
  select(spate, spme, n, teh, measurementnoise, starts_with("modest")) %>%
  pivot_longer(cols = contains("modest_"), names_prefix = "modest_", names_to = "criterion") 

ggplot(mod_estimates, aes(value, fill = as_factor(spme))) +
  geom_histogram(position = "identity", alpha = 0.5) +
  facet_grid(as_factor(measurementnoise) ~ criterion, scales = "free")
```
...
rr_data <- read_csv("../Data/Tautges_LTAR_data.csv") %>%
  ...
  filter(Treatment %in% c("OMT", "CMT")) %>% #conventional and organic maize systems
  filter(depthcode == 1)

change_data <- rr_data %>%
  select(year, depthcode, block, Plot, Treatment, percC) %>%
  pivot_wider(values_from = percC, names_from = year, names_prefix = "TC_") %>%
  mutate(
    TC_diff = TC_2012 - TC_1993, 
    block = as_factor(block)) 
```

```{r baseline}
#there is a very small range of baseline SOC in this data, probably not enough to assess saturation hypothesis
range(change_data$TC_1993)
ggplot(change_data, aes(x = Treatment, y = TC_1993, fill = Treatment)) +
  geom_boxplot()
```


```{r change over time plot}
ggplot(change_data, aes(y = TC_diff, x = Treatment, fill = Treatment)) +
  geom_boxplot()
```


```{r naive plot}
ggplot(change_data, aes(x = TC_1993, y = TC_diff)) +
  geom_point(aes(color = Treatment)) +
  geom_smooth(method = "lm")
```

```{r CATE}
B_grid <- seq(0.8, 1.1, by=0.01) 
alpha <- 0.05 #confidence level

#naive estimate of difference exhibiting "regression to the mean"
naive_estimate <- predict(lm(TC_diff ~ TC_1993, data = change_data), newdata = data.frame(TC_1993 = B_grid), interval = "confidence", level = 1-alpha)

#linear model estimates of CATE with confidence interval
mu_hat_0 <- predict(lm(TC_2012 ~ TC_1993, data = change_data, subset = Treatment == "CMT"), newdata = data.frame(TC_1993 = B_grid), interval = "confidence", level = 1-alpha/2)
mu_hat_1 <- predict(lm(TC_2012 ~ TC_1993, data = change_data, subset = Treatment == "OMT"), newdata = data.frame(TC_1993 = B_grid), interval = "confidence", level = 1-alpha/2)
tau_hat_ols <- cbind(
  "estimate" = mu_hat_1[,1] - mu_hat_0[,1], 
  "lower_bound" = mu_hat_1[,2] - mu_hat_0[,3],
  "upper_bound" = mu_hat_1[,3] - mu_hat_0[,2])
...
#plotting in base R
plot(0,0, type = "n", xlim = range(B_grid), ylim = c(-1,1), xlab = "Baseline %TC", ylab = "CATE")
points(naive_estimate[,1] ~ B_grid, type = 'l', lwd = 2, lty = 'solid', col = 'red')
points(naive_estimate[,2] ~ B_grid, type = 'l', lwd = 1, lty = 'dashed', col = 'red')
points(naive_estimate[,3] ~ B_grid, type = 'l', lwd = 1, lty = 'dashed', col = 'red')
points(tau_hat_ols[,1] ~ B_grid, type = 'l', lwd = 2, lty = 'solid', col = 'blue')
...
# Marin Carbon Project and Sierra Foothills (Ryals et al 2014)
...
```{r visualizing nr data, message = F, warn = F, include = FALSE}
...
mu_hat_1_spline <- predict(lm(Y ~ bs(B, df = 5), data = nr_data, subset = Z == 1), newdata = data.frame(B = b_grid), interval = "confidence", level = 1-alpha/2)
mu_hat_0_spline <- predict(lm(Y ~ bs(B, df = 5), data = nr_data, subset = Z == 0), newdata = data.frame(B = b_grid), interval = "confidence", level = 1-alpha/2)
...
points(tau_hat_spline[,1] ~ b_grid, type = 'l', col = 'blue', lwd = 3, lty = 'solid')
points(tau_hat_spline[,2] ~ b_grid, type = 'l', col = 'blue', lwd = 1, lty = 'dashed')
points(tau_hat_spline[,3] ~ b_grid, type = 'l', col = 'blue', lwd = 1, lty = 'dashed')
```
--->
