...
The strong null posits $\tau_i = 0$ for all $i$. More generally, $\tau_i = \delta$ for all $i$ implies that $\tau(b) = \bar{\tau}$, although the reverse is not true. Take the null hypothesis to be the composite null $H_0: \bigcup_{\delta \in \mathbb{R}} \tau_i = \delta$ and consider alternatives of the form $H_A: \tau_i = f(b_i)$ for some increasing $f$. [What are test statistics against this alternative? Split the data at some value $b$ and test for different means in each group; a linear regression; a shape constrained regression; is there something with power against all increasing $f$?]
