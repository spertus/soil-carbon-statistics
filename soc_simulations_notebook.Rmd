...
- There is measurement error that is only 0 in expectation (hopefully).
...
The second point is non-negotiable. We need multiple replicates of samples in order to estimate the variance of our sample mean: $\hat{\mathbb{V}}[\frac{1}{n} \sum_{i=1}^n z_i] = \frac{\hat{\sigma}^2}{n}$. With only one measured sample $\hat{\sigma}^2$ is not estimable.
...
Under additive measurement error we need $\mathbb{E}[\delta_i] = 0$ for unbiasedness. Under multiplicative error we need $\mathbb{E}[\delta_i] = 1$ and $\delta_i \indep s_i$ (technically we just need $\mbox{Cov}(\delta_i, s_i) = 0$). I believe multiplicative measurement error is more reasonable because errors are likely to enter through errors in the mass, which is itself multiplicative in the determination of %SOC from a chromatographic reading.

#### EcoCore and measurement error

EcoCore analytic services reports a measurement error from DC-EA as a 2.5\% coefficient of variation assuming a mean %SOC of 2.5%. 
