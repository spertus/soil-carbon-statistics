...
I have explored using a Gaussian copula for this, but for now I will use a beta distribution where the mean is a function varying across space. The [beta](https://en.wikipedia.org/wiki/Beta_distribution) is a distribution defined on [0,1], often used to model probabilities or proportions. For $X \sim \mbox{Beta}(\alpha, \beta)$ we have:
...
simulate_truth <- function(){
  ...
  #mean varies across rows, it decreases linearly from .05 to .005
  grid <- (1:n_latitude) / n_latitude
  linear_mean <- .05 * (1-grid) + .005 * grid
  mean_matrix <- matrix(data = rep(linear_mean, n_latitude), nrow = n_latitude, ncol = n_longitude)
  #fix variance initially
  var_matrix <- matrix(.004, nrow = n_latitude, ncol = n_longitude)
...
- Samples are taken along a transect with $n = 9$ samples per plot. It is not clear if these are randomized at all or merely start at one corner.
- Soils are collected using a 7 cm corer, to a depth of 10 cm.
